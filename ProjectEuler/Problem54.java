...
public class Problem54 {
	public static long handValue(String input) {
		long toReturn = royalStraightFlush(input);
		if (toReturn > 0) {
			return toReturn;
		} else if (fourOfAKind(input)) {
			return 8;
		}
		toReturn = fullHouse(input);
		if (toReturn >= 3) {
			return toReturn;
		} else if (twoPairs(input)) {
			return 3;
		} else if (toReturn == 2) {
			return toReturn;
		}
	...
	public static long royalStraightFlush(String input) {
		boolean isStraight = straight(input);
		boolean isFlush = flush(input);
		...
		if (isStraight && isFlush && containsAce) {
			return 10;
	...
	public static boolean fourOfAKind(String input) {
		return (matching(input.charAt(0), input, 4) || matching(input.charAt(3), input, 4));
	}
	
	public static boolean matching(char value, String input, int number) {
		long count = 0;
		for (int i = 0; i < 5; i++) {
			if (value == input.charAt(i * 3)) {
				count++;
			}
		}
		if (count == number) {
	...
	public static long fullHouse(String input) {
		boolean hasTwoOfAKind = onePair(input);
		boolean hasThreeOfAKind = threeOfAKind(input);
	...
	public static boolean flush(String input) {
		char suit = input.charAt(1);
		for (int i = 1; i <= 4; i++) {
			if (suit != input.charAt(i * 3 + 1)) {
				return false;
			}
		}
		return true;
	...
	public static boolean threeOfAKind(String input) {
		return (matching(input.charAt(0), input, 3) || matching(input.charAt(3), input, 3) ||
				(matching(input.charAt(6), input, 3)));
	}
	
	public static boolean twoPairs(String input) {
		long count = 0;
		for (int i = 0; i < 5; i++) {
			if (matching(input.charAt(i * 3), input, 2)) {
				count++;
			}
		}
		return (count == 4);
	...
	public static boolean onePair(String input) {
		for (int i = 0; i < 4; i++) {
			if (matching(input.charAt(i * 3), input, 2)) {
				return true;
			}
	...
	public static boolean straight(String input) {
		for (int i = 0; i < 9; i++) {
			long count = 0;
			for (int j = 0; j < 5; j++) {
				boolean valueFound = false;
				for (int k = 0; k < 5; k++) {
					if (values[i + j] == input.charAt(k * 3)) {
						valueFound = true;
						break;
					}
				}
	...
	public static long highestValue(String input) {
		long toReturn = 0;
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 13; j++) {
				if (input.charAt(i * 3) == values[j]) {
					if (j > toReturn) {
						toReturn = j;
					}
					break;
	...
	public static long fourOfAKindValue(String input) {
		char character = input.charAt(0);
		char secondCharacter = input.charAt(3);
		if (!(character == secondCharacter || character == input.charAt(6))) {
			character = secondCharacter;
		}
		return cardValue(character);
	}
	
	public static long cardValue(char character) {
		for (int i = 0; i < 13; i++) {
	...
	public static long threeOfAKindValue(String input) {
		char character = input.charAt(0);
		if (!(matching(character, input, 3))) {
			character = input.charAt(3);
			if (!(matching(character, input, 3))) {
				character = input.charAt(6);
			}
		}
		return cardValue(character);
	}
	
	public static long twoPairsValue(String input, boolean highestValue) {
		long toReturn = 0; 
		for (int i = 0; i < 5; i++) {
			char character = input.charAt(i * 3);
			long characterValue = cardValue(character);
			if ((matching(character, input, 2)) &&
					((highestValue && toReturn < characterValue) || (!highestValue && toReturn > characterValue))) {
					toReturn = characterValue;
			}
		}
		return toReturn;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 1000; i++) {
			String player1Hand = hands.substring(0, 14);
			String player2Hand = hands.substring(15, 29);
			long player1HandValue = handValue(player1Hand);
			long player2HandValue = handValue(player2Hand);
			if (player1HandValue > player2HandValue) {
				count++;
			} else if (player1HandValue == player2HandValue) {
				if (player1HandValue == 1 && highestValue(player1Hand) > highestValue(player2Hand)) {
					count++;
				} else if (player1HandValue == 1 && highestValue(player1Hand) < highestValue(player2Hand)) {
					// Do nothing
				} else if (player1HandValue == 2 && twoPairsValue(player1Hand, true) > twoPairsValue(player2Hand, true)) {
					count++;
				} else if (player1HandValue == 2 &&
						twoPairsValue(player1Hand, true) < twoPairsValue(player2Hand, true)) {
					// Do nothing
				} else {
					// Do nothing
				}
			...
			if (i != 1000) {
				hands = hands.substring(31);
			}
		}
		System.out.println("Ausgabe: " + count);
	}
	
	/* Remark: I kind of cheated a bit by only making what's necessary for the problem analysis instead of a full-
	 * encompassing program, but screw that. This is difficult and headache-inducing.
	 * Problem solved
	 * 39833rd person to have solved this problem
